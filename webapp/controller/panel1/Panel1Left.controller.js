sap.ui.define([
    ...
    return Controller.extend("ordermanagement.controller.panel1.Panel1Left", {
        onInit: function () {
            var oMultiInput;
        ...
        onValueHelpRequested: function() {
            ...
            if (!this.pDialog) {
                this.pDialog = this.loadFragment({
                    name: "ordermanagement.view.panel1.panel1customerVHDFilterbar"
                });
            }
            ...
            this.pDialog.then(function(oDialog) {
                ...
                oDialog.getTableAsync().then(function (oTable) {
                    ...
                    if(oTable.bindRows) {
                        ...
                        oTable.addColumn(new UIColumn({label: "Customer Code", template: "orderDetails>customerCode"}));
                        oTable.addColumn(new UIColumn({label: "Customer Name", template: "orderDetails>customer"}));
                        oTable.addColumn(new UIColumn({label: "Province", template: "orderDetails>province"}));
                        oTable.addColumn(new UIColumn({label: "City", template: "orderDetails>city"}));
                        oTable.addColumn(new UIColumn({label: "Country", template: "orderDetails>country"}));
                        oTable.addColumn(new UIColumn({label: "PIVA", template: "orderDetails>piva"}));
                        oTable.addColumn(new UIColumn({label: "Commercial Organization", template: "orderDetails>commercialOrg"}));
                        oTable.addColumn(new UIColumn({label: "Distribution Channel", template: "orderDetails>distributionChannel"}));
                        oTable.addColumn(new UIColumn({label: "Weekly Commodity", template: "orderDetails>weeklyCommodity"}));
                    }
                    ...
                    if (oTable.bindItems) {
                        ...
                        oTable.bindAggregation("items", {
                                ...
                                template: new ColumnListItem({
                                    cells: [new Label({text: "{orderDetails>customerCode}"}), new Label({text: "{orderDetails>customer}"})]
                            }),
                            events: {
                                dataReceived: function () {
                                    oDialog.update();
        ...
        onFilterBarSearch: function () {
			...
			aFilters.push(new Filter({
				filters: [
					new Filter({ path: "customerCode", operator: FilterOperator.Contains, value1: sSearchQuery }),
